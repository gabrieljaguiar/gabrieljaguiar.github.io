...
ScrubberView.prototype.createDOM = function () {
    ...
    this.elt.className =
        this.orientation() === "horizontal" ? "scrubber" : "scrubber-vert";
...
ScrubberView.prototype.attachListeners = function () {
    ...
    document.addEventListener("mousemove", function (evt) {
        if (!mousedown) return;
        evt.preventDefault();
        if (self.orientation() === "horizontal") setValueFromPageX(evt.pageX);
        else setValueFromPageY(evt.pageY);
    });
    ...
    document.addEventListener("touchmove", function (evt) {
        ...
        else setValueFromPageY(evt.changedTouches[0].pageY);
    });
